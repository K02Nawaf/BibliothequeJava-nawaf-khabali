...
public class AddEmprunt {

    public static void display() {
        // Create a new stage for the add loan dialog
        Stage dialogStage = new Stage();
        dialogStage.initModality(Modality.APPLICATION_MODAL);
        dialogStage.setTitle("Add Loan");

        // Create UI components for the add loan form
        ...
        addButton.setOnAction(event -> {
            ...
            try (Connection connection = DBConnection.getConnection();
                 PreparedStatement statement = connection.prepareStatement(
                         "INSERT INTO emprunt (date_emprunt, date_retour, statut_emprunt, Adh_num, ISBN) VALUES (?, ?, ?, ?, ?)")) {
                ...
                if (rowsAffected > 0) {
                    showAlert(Alert.AlertType.INFORMATION, "Success", "Loan Added", "The loan has been successfully added.");
                    dialogStage.close();
                    // Refresh data in the TableView after successful addition
                    EmpruntPage.fetchData();
                } else {
                    showAlert(Alert.AlertType.ERROR, "Error", "Failed to Add Loan", "Failed to add the loan. Please try again.");
                }
        ...
        // Create layout for the add loan dialog
